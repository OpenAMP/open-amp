...
void rpmsg_fc_cb(struct rpmsg_endpoint *ept, uint32_t flags, uint32_t src)
{
	(void)ept;

	remote_fc = flags &&  RPMSG_EPT_FC_ON;
	LPRINTF("The remote endpoint %x is in %sable state\r\n", src, remote_fc ? "en" : "dis");
}
...
static int app(struct rpmsg_device *rdev, void *priv)
{
	...
	/* set flow control callback */
	lept.flow_ctrl_cb = rpmsg_fc_cb;
	remote_fc = 0;
