...
typedef int (*rpmsg_cb_t)(struct rpmsg_endpoint *ept, void *data, int len,
			  uint32_t src, void *priv);
typedef void (*rpmsg_destroy_cb_t)(struct rpmsg_endpoint *ept);
...
static inline int rpmsg_send(struct rpmsg_endpoint *ept, const void *data,
			     int len)
{
	return rpmsg_send_offchannel_raw(ept->rp_chnl, ept->src, ept->dst,
					 data, len, RPMSG_TRUE);
}
...
{
	return rpmsg_send_offchannel_raw(ept->rp_chnl, ept->src, dst, data,
					 len, RPMSG_TRUE);
}
...
{
	return rpmsg_send_offchannel_raw(ept->rp_chnl, src, dst, data, len,
					 RPMSG_TRUE);
}
...
static inline int rpmsg_trysendto(struct rpmsg_endpoint *ept, const void *data,
				  int len, uint32_t dst)
{
	return rpmsg_send_offchannel_raw(ept->rp_chnl, ept->src, dst, data, len,
					 RPMSG_FALSE);
}
 ...
 * create additional endpoints by themselves (see rpmsg_create_ept()).
 */
struct rpmsg_endpoint *rpmsg_create_ept(struct rpmsg_virtio *rpmsgv,
					const char *ept_name, uint32_t src,
					uint32_t dest, rpmsg_cb_t *cb,
					rpmsg_destroy_cb_t *destroy_cb,
					void *priv);
