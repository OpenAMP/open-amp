					      ...
					      const char *vq_name)
{
	...
	if (vdev->id.version == 1) {
		...
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_DESC_LOW, (uint32_t)paddr);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_DESC_HIGH, (uint32_t)(paddr >> 32));
		paddr = metal_io_virt_to_phys(vq->shm_io, vq->vq_ring.avail);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_AVAIL_LOW, (uint32_t)paddr);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_AVAIL_HIGH, (uint32_t)(paddr >> 32));
		paddr = metal_io_virt_to_phys(vq->shm_io, vq->vq_ring.used);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_USED_LOW, (uint32_t)paddr);
		virtio_mmio_write32(vdev, VIRTIO_MMIO_QUEUE_USED_HIGH, (uint32_t)(paddr >> 32));
