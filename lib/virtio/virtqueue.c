...
void virtqueue_disable_cb(struct virtqueue *vq)
{
	...
	if (vq->vq_dev->features & VIRTIO_RING_F_EVENT_IDX) {
		if (IS_ENABLED(VIRTIO_DRIVER_SUPPORT) && \
		    vq->vq_dev->role == VIRTIO_DEV_DRIVER) {
			vring_used_event(&vq->vq_ring) =
			    vq->vq_used_cons_idx - vq->vq_nentries - 1;
			VRING_FLUSH(&vring_used_event(&vq->vq_ring),
				    sizeof(vring_used_event(&vq->vq_ring)));
		}
		if (IS_ENABLED(VIRTIO_DEVICE_SUPPORT) && vq->vq_dev->role == VIRTIO_DEV_DEVICE) {
...
static void vq_ring_init(struct virtqueue *vq, void *ring_mem, int alignment)
{
	...
	if (IS_ENABLED(VIRTIO_DRIVER_SUPPORT) && vq->vq_dev->role == VIRTIO_DEV_DRIVER) {
		int i;
